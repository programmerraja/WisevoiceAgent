...
from .config import settings, NormalizationOptions
...
# we spliting large text into chunks based on sentence and convert the text to tokens and checking the lenght because kokor model has token limit

async def smart_split(
    text: str,
    lang_code: str = "en",
    target_min_tokens: int = settings.target_min_tokens,
    target_max_tokens: int = settings.target_max_tokens,
    max_tokens: int = settings.absolute_max_tokens
) -> Generator[str, None, None]:
    """
    Yield text-only chunks (no tokens, no pauses), based on sentence and clause lengths.
    """
    sentences_info = get_sentence_info(text, lang_code)
    current_chunk = []
    current_tokens = 0

    for sentence_text, _, token_count in sentences_info:
        if token_count > max_tokens:
            # Long sentence: split by clauses (commas)
            ...
            for clause in clauses:
                ...
                if not clause:
                    continue
                if clause_tokens + clause_len > max_tokens:
                    if clause_chunk:
                        yield " ".join(clause_chunk).strip()
                    clause_chunk = [clause]
                    clause_tokens = clause_len
                else:
            ...
            if current_tokens + token_count > target_max_tokens and current_tokens >= target_min_tokens:
